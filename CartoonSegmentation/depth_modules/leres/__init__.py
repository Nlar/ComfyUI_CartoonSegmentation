...
def apply_leres(input_image, thr_a: int = 0, thr_b: int = 0, boost: bool = False, device: str = 'cpu'):
    global model, pix2pixmodel
    ...
    with torch.no_grad():
        ...
        if boost:
            depth = estimateboost(input_image, model, 0, pix2pixmodel, max(width, height), device)
        else:
            depth = estimateleres(input_image, model, width, height, device=device)
